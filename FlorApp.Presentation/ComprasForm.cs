...
namespace FlorApp.Presentation
{
    public partial class ComprasForm : Form
    {
        ...
        private async Task CargarOrdenesDeCompraAsync()
        {
            ...
            {
                CustomMessageBoxForm.Show($"Error al cargar órdenes de compra: {ex.Message}", "Error", MessageBoxIcon.Error);
            }
        }
        ...
        private async Task CargarComboBoxesAsync()
        {
            ...
            {
                CustomMessageBoxForm.Show($"Error al cargar datos: {ex.Message}", "Error", MessageBoxIcon.Error);
            }
        }
        ...
        private async void btnGuardarOrden_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBoxForm.Show("Debe seleccionar un proveedor y añadir al menos un producto.", "Datos Incompletos", MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                ...
                CustomMessageBoxForm.Show("Orden de compra guardada exitosamente.", "Éxito", MessageBoxIcon.Information);
                await CargarDatosIniciales();
            }
            catch (Exception ex)
            {
                CustomMessageBoxForm.Show($"Error al guardar la orden: {ex.Message}", "Error", MessageBoxIcon.Error);
            }
        }
        ...
        private async void btnRecibirMercancia_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBoxForm.Show("Por favor, seleccione una orden de la lista.", "Selección Requerida", MessageBoxIcon.Warning);
                return;
            }

            if (CustomConfirmBoxForm.Show("¿Confirma que ha recibido toda la mercancía de esta orden? Esta acción actualizará el stock de los productos.", "Confirmar Recepción") == DialogResult.Yes)
            {
                try
                {
                    await _ordenCompraRepository.MarcarComoRecibidaAsync(_idOrdenSeleccionada.Value);
                    CustomMessageBoxForm.Show("Mercancía recibida y stock actualizado.", "Éxito", MessageBoxIcon.Information);
                    await CargarDatosIniciales();
                }
                catch (Exception ex)
                {
                    CustomMessageBoxForm.Show($"Error al recibir la mercancía: {ex.Message}", "Error", MessageBoxIcon.Error);
                }
            }
        }
    }
